...
define ss::cert($key='',$crt='',$service=true,$ss=true) {
  ...
  if $ss {
    $certificate = op::fetch_cert($name,$ss::ss_username,$ss::ss_password,$ss::ss_hostname)
    $privatekey  = op::fetch_key($name,$ss::ss_username,$ss::ss_password,$ss::ss_hostname)
  } else {
    $certificate = file(
